...
podTemplate(label: 'performace_tests', containers: [
  ...
  node('performace_tests') {
    ...
    stage('Test Project By Slug') {
      container('jmeter') {
        sh "jmeter -n -t projectBySlugTest.jmx -l projectBySlugTest.jtl -Jthreads 100 -Jduration 300"
    ...
    stage('Test All Project Balances') {
      container('jmeter') {
        sh "jmeter -n -t allProjectsBalancesTest.jmx -l allProjectsBalancesTest.jtl -Jthreads 100 -Jduration 300"
